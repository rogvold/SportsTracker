...
var SportHelper = {
    ...
    generateUsersBySessions: function(sessions){
        console.log('generateUsersBySessions: sessions = ', sessions);
        var oldT = (new Date().getTime()) - 86400000 * 1000;
        var arr = [];
        for (var i in sessions){
            var s = sessions[i].session;
            var startTimestamp = s.startTimestamp;
            if (startTimestamp < oldT){
    ...
    getScaledPoints: function(points, field, maxWidth){
        console.log('getScaledPoints: points, field = ', points, field);
        if (field == undefined){
            return points;
        }
        ...
        var maxHeight = 1.0 * maxWidth * field.width / field.height;
        var kx = 1.0 * maxWidth / field.width;
        var ky = 1.0 * maxHeight / field.height;
        for (var i in points){
    ...
    generateRandomShots: function(n){
        console.log('generateRandomShots occured');
        if (n == undefined){
            n = 30;
        }
        var arr = [];
        for (var i = 0; i < n; i++){
            var k = Math.round(8 * Math.random());
            var left = (Math.random() < 0.5);
            arr.push({
                direction: (left == true) ? 'left' : 'right',
                number: k
            });
        }
        console.log('returning arr = ', arr);
